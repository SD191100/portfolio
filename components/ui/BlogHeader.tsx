...
const BlogHeader = ({ name, category, date, slug }: { name: string; category: string; date: string; slug: string }) => {
  return (
    <tr className="hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors gap-2">
      <td className="px-6 py-4 text-xl text-black">
        <Link href={`/this-pc/blogs/${slug}`} className='text-black dark:text-white  no-underline  hover:text-inherit flex space-x-4 '>
